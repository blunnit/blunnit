          ...
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', minHeight: '100vh', textAlign: 'center', paddingTop: (isDesktop ? 28 : 'calc(env(safe-area-inset-top) + 60px)') as any, paddingBottom: 40 }}>
            ...
            <div style={{ width: '100%', marginBottom: 20 }}>
              {remaining <= 0 && tier !== 'paid' ? (
                ...
                <>
                  <textarea value={journalText} onChange={(e) => setJournalText(e.target.value)} onKeyDown={handleKeyDown} placeholder="What are you bringing to the Mirror?" rows={3} disabled={isReflecting} style={{ width: '100%', display: 'block', background: '#111111', border: '1px solid #222222', color: 'var(--text)', fontSize: 15, lineHeight: 1.6, padding: '12px 16px', fontFamily: F, fontWeight: 300, resize: 'none', outline: 'none', boxShadow: 'none', WebkitAppearance: 'none', boxSizing: 'border-box', transition: 'border-color 0.3s', minHeight: 52, overflowY: 'auto', verticalAlign: 'top', opacity: isReflecting ? 0.5 : 1 }} onFocus={(e) => { e.target.style.borderColor = '#333333'; }} onBlur={(e) => { e.target.style.borderColor = '#222222'; }} />
              ...
              <div style={{ maxWidth: isDesktop ? 720 : 520, margin: '0 auto' }}>
                ...
                <div style={{ display: 'flex', gap: 8 }}>
                  {!user && anonRemaining <= 0 ? (
                    ...
                    <>
                      <textarea value={journalText} onChange={(e) => setJournalText(e.target.value)} onKeyDown={handleKeyDown} placeholder="What are you bringing to the Mirror?" rows={3} disabled={isReflecting} style={{ flex: 1, display: 'block', background: '#111111', border: '1px solid #222222', color: 'var(--text)', fontSize: 15, lineHeight: 1.6, padding: '12px 16px', fontFamily: F, fontWeight: 300, resize: 'none', outline: 'none', boxShadow: 'none', WebkitAppearance: 'none', boxSizing: 'border-box', transition: 'border-color 0.3s', minHeight: 52, overflowY: 'auto', verticalAlign: 'top', opacity: isReflecting ? 0.5 : 1 }} onFocus={(e) => { e.target.style.borderColor = '#333333'; if (!isDesktop) { setTimeout(() => e.target.scrollIntoView({ behavior: 'smooth', block: 'nearest' }), 300); } }} onBlur={(e) => { e.target.style.borderColor = '#222222'; }} />
