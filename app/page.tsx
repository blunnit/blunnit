              ...
              <div style={{ maxWidth: isDesktop ? 720 : 520, margin: '0 auto' }}>
                ...
                <div style={{ display: 'flex', gap: 8 }}>
                  {!user && anonRemaining <= 0 ? (
                    ...
                    <>
                      <textarea value={journalText} onChange={(e) => setJournalText(e.target.value)} onKeyDown={handleKeyDown} placeholder="What are you bringing to the Mirror?" rows={isDesktop ? 3 : 2} disabled={isReflecting} style={{ flex: 1, background: '#1a1a1a', border: '1px solid #2a2a2a', color: 'var(--text)', fontSize: 15, lineHeight: 1.6, padding: '14px 16px', fontFamily: F, fontWeight: 300, resize: 'none', outline: 'none', boxShadow: 'none', WebkitAppearance: 'none', boxSizing: 'border-box', transition: 'border-color 0.3s', opacity: isReflecting ? 0.5 : 1 }} onFocus={(e) => { e.target.style.borderColor = '#3a3a3a'; if (!isDesktop) { setTimeout(() => e.target.scrollIntoView({ behavior: 'smooth', block: 'nearest' }), 300); } }} onBlur={(e) => { e.target.style.borderColor = '#2a2a2a'; }} />
