          ...
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', minHeight: '100vh', textAlign: 'center', paddingTop: (isDesktop ? 28 : 'calc(env(safe-area-inset-top) + 60px)') as any, paddingBottom: 40 }}>
            ...
            <div style={{ width: '100%', marginBottom: 20 }}>
              {remaining <= 0 && tier !== 'paid' ? (
                ...
                <>
                  <textarea value={journalText} onChange={(e) => setJournalText(e.target.value)} onKeyDown={handleKeyDown} placeholder="What are you bringing to the Mirror?" rows={3} disabled={isReflecting} style={{ width: '100%', display: 'block', background: '#1a1a1a', border: '1px solid #2a2a2a', color: 'var(--text)', fontSize: 15, lineHeight: 1.6, padding: '12px 16px', fontFamily: F, fontWeight: 300, resize: 'none', outline: 'none', boxShadow: 'none', WebkitAppearance: 'none', boxSizing: 'border-box', transition: 'border-color 0.3s', minHeight: 52, overflowY: 'auto', opacity: isReflecting ? 0.5 : 1 }} onFocus={(e) => { e.target.style.borderColor = '#3a3a3a'; }} onBlur={(e) => { e.target.style.borderColor = '#2a2a2a'; }} />
              ...
              <div style={{ maxWidth: isDesktop ? 720 : 520, margin: '0 auto' }}>
                ...
                <div style={{ display: 'flex', gap: 8 }}>
                  {!user && anonRemaining <= 0 ? (
                    ...
                    <>
                      <textarea value={journalText} onChange={(e) => setJournalText(e.target.value)} onKeyDown={handleKeyDown} placeholder="What are you bringing to the Mirror?" rows={3} disabled={isReflecting} style={{ flex: 1, display: 'block', background: '#1a1a1a', border: '1px solid #2a2a2a', color: 'var(--text)', fontSize: 15, lineHeight: 1.6, padding: '12px 16px', fontFamily: F, fontWeight: 300, resize: 'none', outline: 'none', boxShadow: 'none', WebkitAppearance: 'none', boxSizing: 'border-box', transition: 'border-color 0.3s', minHeight: 52, overflowY: 'auto', opacity: isReflecting ? 0.5 : 1 }} onFocus={(e) => { e.target.style.borderColor = '#3a3a3a'; if (!isDesktop) { setTimeout(() => e.target.scrollIntoView({ behavior: 'smooth', block: 'nearest' }), 300); } }} onBlur={(e) => { e.target.style.borderColor = '#2a2a2a'; }} />
                      <button onClick={handleReflect} disabled={!journalText.trim() || isReflecting} style={{ padding: '14px 20px', background: journalText.trim() && !isReflecting ? 'var(--btn-bg)' : '#1a1a1a', color: journalText.trim() && !isReflecting ? 'var(--btn-text)' : 'var(--text-muted)', border: `1px solid ${journalText.trim() && !isReflecting ? 'var(--btn-bg)' : '#2a2a2a'}`, fontSize: 13, letterSpacing: 2, textTransform: 'uppercase', cursor: journalText.trim() && !isReflecting ? 'pointer' : 'default', fontFamily: F, fontWeight: 500, transition: 'all 0.3s ease', whiteSpace: 'nowrap' }}>{isReflecting ? '...' : 'Reflect'}</button>
                    </>
                  )}
                </div>
