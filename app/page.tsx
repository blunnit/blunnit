    ...
    <div style={{ minHeight: '100vh', position: 'relative', fontFamily: F }}>
      ...
      <div style={{ position: 'relative', zIndex: 2, maxWidth: isDesktop && screen !== 'disclaimer' ? 800 : 520, margin: '0 auto', padding: isDesktop && screen !== 'disclaimer' ? '0 40px' : '0 28px' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', minHeight: '100vh', textAlign: 'center', paddingTop: (isDesktop ? 28 : 'calc(env(safe-area-inset-top) + 60px)') as any, paddingBottom: 40 }}>
            ...
            <div style={{ width: '100%', marginBottom: 20 }}>
              {remaining <= 0 && tier !== 'paid' ? (
                ...
                <>
                  <textarea value={journalText} onChange={(e) => setJournalText(e.target.value)} onKeyDown={handleKeyDown} placeholder="What are you bringing to the Mirror?" rows={3} disabled={isReflecting} style={{ width: '100%', display: 'block', background: '#111111', border: '1px solid #222222', borderRadius: 8, color: 'var(--text)', fontSize: 16, lineHeight: 1.4, padding: '14px 16px', fontFamily: F, fontWeight: 300, resize: 'none', outline: 'none', boxShadow: 'none', WebkitAppearance: 'none', boxSizing: 'border-box', transition: 'border-color 0.3s', minHeight: 48, verticalAlign: 'top', opacity: isReflecting ? 0.5 : 1 }} onFocus={(e) => { e.target.style.borderColor = '#333333'; }} onBlur={(e) => { e.target.style.borderColor = '#222222'; }} />
            ...
            <button onClick={handleReflect} disabled={!journalText.trim() || isReflecting || (remaining <= 0 && tier !== 'paid')} style={{ width: '100%', padding: '14px 20px', background: journalText.trim() && !isReflecting && (remaining > 0 || tier === 'paid') ? 'var(--btn-bg)' : '#1a1a1a', color: journalText.trim() && !isReflecting && (remaining > 0 || tier === 'paid') ? 'var(--btn-text)' : 'var(--text-muted)', border: `1px solid ${journalText.trim() && !isReflecting && (remaining > 0 || tier === 'paid') ? 'var(--btn-bg)' : '#2a2a2a'}`, fontSize: 13, letterSpacing: 2, textTransform: 'uppercase', cursor: journalText.trim() && !isReflecting && (remaining > 0 || tier === 'paid') ? 'pointer' : 'default', fontFamily: F, fontWeight: 500, transition: 'all 0.3s ease' }}>{isReflecting ? 'Reflecting...' : 'Reflect'}</button>
        ...
        {screen === 'mirror' && (<>
          <div style={{ display: 'flex', flexDirection: 'column', height: '100dvh' }}>
            ...
            <div
              onScroll={(e) => {
                ...
                const hasScrolledUpSignificantly = distanceFromBottom > 200 && scrollTop < lastScrollTopRef.current;
                if (distanceFromBottom < 200) {
                  setBackBtnVisible(false);
                } else if (isNearTop || hasScrolledUpSignificantly) {
                  setBackBtnVisible(true);
                }
                lastScrollTopRef.current = scrollTop;
              }}
              style={{ flex: 1, overflowY: 'auto', overscrollBehavior: 'none', paddingTop: (isDesktop ? '100px' : 'calc(env(safe-area-inset-top) + 164px)') as any, paddingBottom: 150 }}
            >
          ...
          {/* Input bar — position: fixed, always anchored to bottom */}
          <div style={{ position: 'fixed', bottom: 0, left: isDesktop && sidebarOpen ? 280 : 0, right: 0, background: '#000000', paddingBottom: 'env(safe-area-inset-bottom)' as any, zIndex: 40 }}>
              ...
              {!user && anonRemaining <= 0 ? (
                ...
                <>
                  <textarea
                    ...
                    onFocus={(e) => { e.target.style.borderColor = '#333333'; setTimeout(() => { textareaRef.current?.scrollIntoView({ behavior: 'instant' as any, block: 'nearest' }); }, 300); }}
